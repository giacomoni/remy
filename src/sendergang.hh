...
public:
    TimeSwitchedSender(const unsigned int s_id, const double& start_tick, const SenderType& s_sender) :
    SwitchedSender<SenderType>(s_id, start_tick, s_sender) {
    }
...
private:
    unsigned int packets_sent_cap_;

public:

    ByteSwitchedSender(const unsigned int s_id, const double& start_tick, const SenderType& s_sender, unsigned int packets_sent_cap_) :
    SwitchedSender<SenderType>(s_id, start_tick, s_sender), packets_sent_cap_(packets_sent_cap_) {
    }
...
public:
    ...
    ExternalSwitchedSender(const unsigned int s_id, const double& start_tick, const SenderType& s_sender) :
    SwitchedSender<SenderType>(s_id, start_tick, s_sender) {
    }
    template <class NextHop>
    void tick(NextHop & next, Receiver & rec,
...
public:
    ...
    size_t count_senders(void) const {
        return _gang.size();
    }

    unsigned int id_of_first_sender(void) const {
        return _gang.at(0).id;
    }
    ...
    SwitcherType & mutable_sender(const unsigned int num) {
        return _gang.at(num);
    }

    const SwitcherType & sender(const unsigned int num) const {
        return _gang.at(num);
    }
};
